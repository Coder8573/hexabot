...
#
    ...
    def get_rotations_angle(self, leg, x):
        ...
        if (math.sin(math.radians(config.data["legMountAngle"][leg - 1])) * (self.origin_point[0] + 120))-rotation_point[0] > 0:
            rotation_angle = 270 - math.degrees(math.atan((math.cos(math.radians(
                config.data["legMountAngle"][leg - 1])) * (self.origin_point[0] + 120) - rotation_point[1]) / (math.sin(math.radians(
                config.data["legMountAngle"][leg - 1])) * (self.origin_point[0] + 120) - rotation_point[0])))
        elif (math.sin(math.radians(config.data["legMountAngle"][leg - 1])) * (self.origin_point[0] + 120))-rotation_point[0] < 0:
            rotation_angle = 90 - math.degrees(math.atan((math.cos(math.radians(
                config.data["legMountAngle"][leg - 1])) * (self.origin_point[0] + 120) - rotation_point[1]) / (math.sin(math.radians(
                config.data["legMountAngle"][leg - 1])) * (self.origin_point[0] + 120) - rotation_point[0])))
        else:
            rotation_angle = 0
