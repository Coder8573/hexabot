...
class Control:
    def __init__(self, port):
        ...
        self.walk_progess = []
        self.inverted = False
        self.last_local_point = [150, 0, -160]
        #angles = self.read_pos()
        #for leg in range(int(len(angles) / 3)):
        #    self.last_points[leg-1] = self.calc.angels_to_coord(angles[leg * 3], angles[leg * 3 + 1], angles[leg * 3 + 2], leg)
        #    self.last_points[leg-1] = self.calc.local_coord_to_abs_coord(self.last_points[leg-1], leg)
    ...
    def move(self, leg, coord):
        steps = self.calc.abs_coord_to_steps(coord, leg)
    ...
    def pre_move_local_coord(self, leg, coord):  # , direction, speed, rotation
        steps = self.calc.local_coord_to_steps(coord, leg)
        if not self.packet:
    ...
    def walk(self, dir, speed, method=1):
        if method == 0:
             ...
             self.last_local_point, self.walk_progess, self.inverted = walk.gen_next_point(1, dir, [160, 0, -160], self.last_local_point, stepsize, self.walk_progess, self.inverted)

             self.pre_move_local_coord(1, self.last_local_point)
             self.pre_move_local_coord(2, self.last_local_point)
             self.pre_move_local_coord(3, self.last_local_point)
             self.pre_move_local_coord(4, self.last_local_point)
             self.pre_move_local_coord(5, self.last_local_point)
             self.pre_move_local_coord(6, self.last_local_point)
             self.execute_move()
...
#
    ...
    def plot_walk(self):
        ...
        for dir in range(180, 360, 90):
            for i in range(0, len(self.walk_points)):
                ...
                gen_points.append(self.calc.abs_coord_to_local_coord(walk.gen_next_point(1, dir, [160, 0, -160], self.next_point, self.walk_points), 1))
                gen_points.append(self.calc.abs_coord_to_local_coord(walk.gen_next_point(2, dir, [160, 0, -160], self.next_point, self.walk_points), 2))
                gen_points.append(self.calc.abs_coord_to_local_coord(walk.gen_next_point(3, dir, [160, 0, -160], self.next_point, self.walk_points), 3))
                gen_points.append(self.calc.abs_coord_to_local_coord(walk.gen_next_point(4, dir, [160, 0, -160], self.next_point, self.walk_points), 4))
                gen_points.append(self.calc.abs_coord_to_local_coord(walk.gen_next_point(5, dir, [160, 0, -160], self.next_point, self.walk_points), 5))
                gen_points.append(self.calc.abs_coord_to_local_coord(walk.gen_next_point(6, dir, [160, 0, -160], self.next_point, self.walk_points), 6))
                self.next_point = self.next_point + 1
    ...
    def display_robot_movement(self):
        angles = self.read_pos()
        for leg in range(int(len(angles)/3)):
            print(self.calc.angels_to_coord(angles[leg*3], angles[leg*3+1], angles[leg*3+2], leg))
