...
def gen_foot_path(walk_progress):
    point = gen_point(((walk_progress-25)/50)*curve_top_len, curve_top_len, curve_top_boundary)
    return point
...
def gen_next_path_point(dir, stepsize, last_point, walk_progress):
    if walk_progress <= 25 or walk_progress > 75:
        point = [last_point[0] + math.sin(math.radians(dir)) * stepsize,
                 last_point[1] + math.cos(math.radians(dir)) * stepsize,
                 last_point[2]]
        walk_point = point
        distance_from_middle = difference_between_two_points(point[0:-1], [0, 0])
        walk_progress = (distance_from_middle/step_length)*50
    else:
        #last_point = [0, 0, 0]
        point = [last_point[0] - math.sin(math.radians(dir)) * stepsize,
                 last_point[1] - math.cos(math.radians(dir)) * stepsize,
                 last_point[2]]
        distance_from_middle = difference_between_two_points(point[0:-1], [0, 0])
        walk_dir = math.degrees(math.asin(point[0]/distance_from_middle))
        point_on_perimeter = [math.sin(math.radians(walk_dir))*(0.5*step_length), math.cos(math.radians(walk_dir))*(0.5*step_length)]
        distance_from_circle = difference_between_two_points(point_on_perimeter, point)
        #print(f"walk dir: {walk_dir}")
        #print(point)
        walk_progress = (distance_from_circle / step_length) * 50 + 25
        #print(walk_progress)
        walk_point = gen_foot_path(walk_progress)
        walk_point = [math.sin(math.radians(dir))*walk_point[0],
                      -math.cos(math.radians(dir))*walk_point[0],
                      walk_point[1]]
        print(walk_point)
    ...
    return walk_point, point, walk_progress
